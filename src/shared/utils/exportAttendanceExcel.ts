...
const COLUMNS: Column<RosterEntry>[] = [
  ...
  {
    header: header('Hora de ingreso'),
    cell: (entry) => ({
      value: entry.attendedDate
        ? new Date(entry.attendedDate).toLocaleString('es-CL', {
            day: '2-digit',
            month: '2-digit',
            year: 'numeric',
            hour: '2-digit',
...
export async function exportAttendanceExcel(event: Event, roster: RosterEntry[]): Promise<void> {
  const attendees = roster.filter((entry) => entry.attended)
  const fileName = `asistencia-${sanitizeFileNamePart(event.name)}.xlsx`
  await writeExcelFile(attendees, { columns: COLUMNS }).toFile(fileName)
}
